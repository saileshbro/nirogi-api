...
module.exports.myProfile = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            "SELECT user_id,name,imageUrl,address,created_at FROM users WHERE user_id=?",
            [user_id]
        );
        if (result.length == 0) {
...
module.exports.updateImage = async (req, res) => {
    ...
    try {
        const result = await pool.query("UPDATE users SET imageUrl=? WHERE user_id=?", [req.file.destination + req.file.filename, user_id]);
        if (result.affectedRows == 1) {
            return res.send({
