...
module.exports.addFood = async (req, res) => {
  ...
  try {
    const result = await pool.query("INSERT INTO food SET name=?,imageUrl=?", [
      name,
      imageUrl
    ]);
    if (!result) {
    ...
    if (toeat.length > 0) {
      for (let i = 0; i < toeat.length; i++) {
        const insert = await pool.query(
          "INSERT INTO tips SET food_id=?,disease_id=?,value=?",
          [toeat[i], disease_id, 1]
        );
...
module.exports.getTips = async (req, res) => {
  const disease_id = parseInt(req.params.disease_id);
  try {
    const result = await pool.query(
