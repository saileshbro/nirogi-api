...
var upload = multer({
    ...
    fileFilter: function (req, file, callback) {
        if (fs.existsSync(`uploads/${file.fieldname + req.user.user_id + '.jpg'}`)) {
            fs.unlinkSync(`uploads/${file.fieldname + req.user.user_id + '.jpg'}`);
        }
        if (fs.existsSync(`uploads/${file.fieldname + req.user.user_id + '.png'}`)) {
            fs.unlinkSync(`uploads/${file.fieldname + req.user.user_id + '.png'}`);
        }
        if (fs.existsSync(`uploads/${file.fieldname + req.user.user_id + '.jpeg'}`)) {
            fs.unlinkSync(`uploads/${file.fieldname + req.user.user_id + '.jpeg'}`);
        }
